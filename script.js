...
// ลบโค้ดถาวรออก เหลือแต่โค้ดเวลาเท่านั้น
...
function setupEventListeners() {
    ...
    document.getElementById('check-codes-btn').addEventListener('click', checkMyUsedCodes);
    document.getElementById('history-btn').addEventListener('click', showHistoryModal);
    document.getElementById('generate-codes-btn').addEventListener('click', generateCustomCodes);
...
function processTopup() {
    ...
    if (selectedOption) {
        ...
        // ตรวจสอบเฉพาะโค้ดเวลา (โค้ดถาวรถูกลบออกแล้ว)
...
// ฟังก์ชันอัพเดทประวัติการทำรายการใน Modal
function updateTransactionHistoryModal() {
    const historyList = document.getElementById('history-list');
    const transactions = users[currentUser].transactions || [];
    
    if (transactions.length === 0) {
        historyList.innerHTML = '<p>ยังไม่มีประวัติการทำรายการ</p>';
        return;
    }
...
// ฟังก์ชันคัดลอกโค้ดทั้งหมด
function copyAllCodes() {
    const codesList = document.getElementById('codes-list');
    const codesText = codesList.textContent;
    
    navigator.clipboard.writeText(codesText).then(() => {
        alert('คัดลอกโค้ดทั้งหมดเรียบร้อยแล้ว!');
    }).catch(err => {
...
function addAdminButtons() {
    if (currentUser === ADMIN_USERNAME) {
        ...
        adminSection.innerHTML = `
            ...
            <button id="admin-generate-codes-btn">สร้างโค้ด</button>
        ...
        // เพิ่ม event listener สำหรับปุ่มสร้างโค้ดของแอดมิน
        document.getElementById('admin-generate-codes-btn').addEventListener('click', function() {
...
function viewAllGeneratedCodes() {
    ...
    message += `🔴 ใช้แล้ว: ${usedCodesList.length} รหัส\n`;
...
function cleanupExpiredCodes() {
    if (currentUser !== ADMIN_USERNAME) {
        alert('เฉพาะแอดมินเท่านั้นที่สามารถลบโค้ดได้');
        return;
    }
